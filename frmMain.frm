...
Select Case KeyCode
    ...
    If NumPlayers >= 1 Then If (FldY(PHX(2), PHY(2)) = -1 + ConPlayer(2) Or Not CanChange(2)) And (DontStop = 0 Or DontStop = 2) Then Exit Sub
...
Do While Not MDetect = "" 'determine max number of maps
...
Dim Crashed As Boolean
    ...
    'remember: "PHY(I) - (FldY(PHX(I), PHY(I)) - ConPlayer(I))" computes the next Y position (same for X)
    '"FldX(PHX(I) - (FldX(PHX(I), PHY(I)) - ConPlayer(I)), PHY(I) - (FldY(PHX(I), PHY(I)) - ConPlayer(I)))" (aaah!) computes the thing in the next X position (same for Y)
    
    For I = IIf(StopNum > 0, DontStop, 1) To IIf(StopNum > 0, DontStop, NumPlayers)
        temp = PHX(I) - (FldX(PHX(I), PHY(I)) - ConPlayer(I))
        If temp > 88 Or temp < 1 Then 'here we check whether a snake will
            Crashed = True 'crash with a boundary next move.
            PyCrashed(I) = True 'if, set the variable
        End If
        temp = PHY(I) - (FldY(PHX(I), PHY(I)) - ConPlayer(I))
        If temp > 63 Or temp < 1 Then
            Crashed = True 'this is a flag
            PyCrashed(I) = True
        End If
    Next I
    ...
    For I = IIf(StopNum > 0, DontStop, 1) To IIf(StopNum > 0, DontStop, NumPlayers)
        If Not PyCrashed(I) Then 'here we check if next move, the snake will crash on a wall, or will collect a power-up
            temp = FldX(PHX(I) - (FldX(PHX(I), PHY(I)) - ConPlayer(I)), PHY(I) - (FldY(PHX(I), PHY(I)) - ConPlayer(I)))
...
For I = 1 To 88
    For J = 1 To 63
        Select Case FldX(I, J)
            ...
            pGame.PaintPicture LoadPicture(App.Path + "\wres" & FldX(I, J) & ".bmp"), QX(I), QY(J)
            Case fldMI1
            pGame.PaintPicture LoadPicture(App.Path + "\wres-7.bmp"), QX(I), QY(J)
            Case fldMI2
            pGame.PaintPicture LoadPicture(App.Path + "\wres-9.bmp"), QX(I), QY(J)
            Case fldMI3
            pGame.PaintPicture LoadPicture(App.Path + "\wres-7.bmp"), QX(I), QY(J) + 9, 10, -10
            Case fldMI4
            pGame.PaintPicture LoadPicture(App.Path + "\wres-9.bmp"), QX(I) + 9, QY(J), -10, 10
            Case fldMRot1
            pGame.PaintPicture LoadPicture(App.Path + "\wres-3.bmp"), QX(I), QY(J)
            Case fldMRot2
            pGame.PaintPicture LoadPicture(App.Path + "\wres-3.bmp"), QX(I) + 9, QY(J), -10, 10
            Case fldMRot3
            pGame.PaintPicture LoadPicture(App.Path + "\wres-3.bmp"), QX(I) + 9, QY(J) + 9, -10, -10
            Case fldMRot4
            pGame.PaintPicture LoadPicture(App.Path + "\wres-3.bmp"), QX(I), QY(J) + 9, 10, -10
        End Select
    Next J
Next I
End Sub
...
Dim ff As Integer, tmp As String * 1, I%, J%
...
For I = IIf(StopNum > 0, DontStop, 1) To IIf(StopNum > 0, DontStop, NumPlayers)
    ...
    FldX(PHX(I) - (FldX(PHX(I), PHY(I)) - ConPlayer(I)), PHY(I) - (FldY(PHX(I), PHY(I)) - ConPlayer(I))) = FldX(PHX(I), PHY(I))
    FldY(PHX(I) - (FldX(PHX(I), PHY(I)) - ConPlayer(I)), PHY(I) - (FldY(PHX(I), PHY(I)) - ConPlayer(I))) = FldY(PHX(I), PHY(I))
    'move the head to the new position
    PHX(I) = PHX(I) - (FldX(tmpHTX, tmpHTY) - ConPlayer(I))
    PHY(I) = PHY(I) - (FldY(tmpHTX, tmpHTY) - ConPlayer(I))
    'draw new head
    BitDraw PHX(I), PHY(I)
    ...
    If Longer(I) = 0 Then
        ...
        PTX(I) = PTX(I) - (FldX(tmpHTX, tmpHTY) - ConPlayer(I)) 'QUIIII!
        PTY(I) = PTY(I) - (FldY(tmpHTX, tmpHTY) - ConPlayer(I))
    ...
    If Shorten(I) > 0 Then
        ...
        PTX(I) = PTX(I) - (FldX(tmpHTX, tmpHTY) - ConPlayer(I))
        PTY(I) = PTY(I) - (FldY(tmpHTX, tmpHTY) - ConPlayer(I))
        FldX(tmpHTX, tmpHTY) = -2
        FldY(tmpHTX, tmpHTY) = -2
        BitBlt pGame.hDC, QX(tmpHTX), QY(tmpHTY), 10, 10, pblack.hDC, 0, 0, SRCCOPY
...
Select Case pwrUp
    Case fldApple
        ...
        If ExpGrowing Then
            ...
            If PyLen(py) >= 40 Then Longer(py) = 30 Else Longer(py) = PyLen(py)
    ...
    Case fldStop
        ...
        If PyLen(py) > 40 Then StopNum = PyLen(py) Else StopNum = 40
...
For K = 1 To NumPlayers
    ...
    Do
        ...
        CX = CX - (temp1 - ConPlayer(K))
        CY = CY - (temp2 - ConPlayer(K))
